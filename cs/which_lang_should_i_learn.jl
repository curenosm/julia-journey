...
languages = ["Julia", "Wolfram", "Python", "Lean4", "Java", "Kotlin", "Clojure",
    "Rust", "Scala", "SQL", "R", "C", "C++"
]
...
begin
    ...
    @constraint(model, sum(x[i] for i in 1:N) <= capacity)
    @expression(model, speed_expr, sum(speed[i] * x[i] for i in 1:N))
    @expression(model, ease_of_use_expr, sum(ease_of_use[i] * x[i] for i in 1:N))
    @expression(model, science_expr, sum(scientific_domains[i] * x[i] for i in 1:N))

...
checktype(sqrt(2)) # or we can also use the symbol √
checktype(√2)
#> Look! A Float!
...
# For simple logical statement, one can be more terse using the 
# "ternary operator", which takes the form
# `predicate ? do_if_true : do_if_false`
...
# The operators `&&` for AND and `||` for OR only evaluate the right-hand

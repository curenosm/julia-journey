...
m4 = [i + j + k for i = 1:2, j = 1:3, k = 1:2] # creates a 2x3x2 array
...
m5 = LegacyStrings.ASCIIString[
  "Hi Im element # $(i+2*(j-1 + 3*(k - 1)))" for i = 1:2, j = 1:3, k = 1:2
]
printsum(m5)
...
findmax(m4, dims = (1, 3)) # sum over first and third dim
# (available only in very recent versions of Julia)

# Broadcasting
# when you combine arrays of different sizes in an operation,
# an attempt is made to "spread" or "broadcast" the smaller array
# so that the sizes match up. broadcast operators are preceeded by a dot:
...
m4[:, 2, :] # that's a 2x1x2 array. not very intuitive to look at

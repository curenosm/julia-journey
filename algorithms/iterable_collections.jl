...
unique([[12, 2, 55, 1], [1, 1, 2, 5], [8, 8, 2, 1], [1, 1, 1, 1]], dims = 2)
...
matrix = [[12, 2, 55, 1], [1, 1, 2, 5], [8, 8, 2, 1], [1, 1, 1, 1]]
...
matrix_of_ranges = [i:0.1:j for i = 1:30, j = 1:30]
...
function my_exp(x, n::Integer, factorial = my_factorial)
  reduce(+, [x^i / factorial(i) for i = 0:n])
end

my_exp(1, 10) # 2.7182818284590455

# Using generators, which do not allocate memory
my_exp2(x, n::Integer) = sum(x^i / my_factorial(i) for i = 0:n)

...
function main()
  ...
  interpolator = interpolate((x_interval, y_interval), velocity_x, Gridded(Linear()))

  theme(:dark)

  @showprogress "Timestepping ..." for iter = 1:N_TIME_STEPS
    # (1) Apply the forces
    velocity_x_prev .+= force_x * TIME_STEP_LENGTH

    # (2) Self-Advection by backtracing and interpolation
    backtraced_coordinates_x .=
      mod1.(coordinates_x - TIME_STEP_LENGTH * velocity_x_prev, x_extent)
    backtraced_coordinates_y .=
      mod1.(coordinates_y - TIME_STEP_LENGTH * velocity_y_prev, 1.0)

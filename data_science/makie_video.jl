...
# ╔═╡ f3abbe3a-45d1-11ef-0a03-5d030b999bc0
begin
  using Dates
  using DataFrames
  using GLMakie
...
# ╔═╡ e616dd30-1c42-421b-b732-5afd40794646
begin
  const FILES_DIR::String = "data"
end

# ╔═╡ e00c8380-424f-467f-a652-1402a2bb511d
"""
    generate_file_name(custom_id::String, extension::String)::String

Function used to generate a file name based on the current date and time.
"""
function generate_file_name(custom_id::String, extension::String)::String
  "$(Dates.format(Dates.now(), "HHMMSS_s"))_$(custom_id).$(extension)"
end

# ╔═╡ 704bcaf2-9af2-4482-801d-476ca0070895
@bind xp PlutoUI.Slider(0:0.01:3, default = 1)

# ╔═╡ 2d838663-367d-44d7-b3b6-bbecfa3e75e8
@bind yp PlutoUI.Slider(0:0.01:3, default = 1)

# ╔═╡ 54626293-28ed-463a-ad47-0d13104e91bf
@bind zp PlutoUI.Slider(0:0.01:3, default = 1)

# ╔═╡ 721fd23a-478a-41cf-8f95-569d5819bdbc
@bind azimuth NumberField(0:0.05:100, default = 0.5)

# ╔═╡ c4f8a9df-3870-4e92-8ea2-4afbe473507d
# @bind cmap Select([:viridis, :heat, :greens, :blues])
@bind cmap Select([
  :viridis => "🍎",
  :heat => "CALIDO",
  :greens => "VERDOSO",
...
# ╔═╡ fca7efee-fb7e-49f1-bfc3-eaf9d1b31c0d
@bind switch_color CheckBox()

# ╔═╡ 25170648-4922-4a32-9940-c9d562b50e1e
@bind plot_title TextField()

# ╔═╡ 78c5bfcc-9169-448a-842c-efbd231fd435
function surface(extension::String = "png")
  global xp, yp, zp, azimuth, switch_color, plot_title, cmap

  x = y = LinRange(-2, 2, 51)
  z = (-x .* exp.(-x .^ 2 .- (y') .^ 2)) .* 4
  zmin, zmax = minimum(z), maximum(z)
  panel_color = if switch_color
    :black
  else
    :transparent
  end
  ...
  ax = Axis3(
    ...
    perspectiveness = 0.5,
    elevation = π / 9,
    xzpanelcolor = (:transparent, 0.75),
    yzpanelcolor = (panel_color, 0.75),
    zgridcolor = :grey,
    ygridcolor = :grey,
    xgridcolor = :grey;
    azimuth,
  )
  point = Point3f(xp, yp, zp)
  scatter!(point, color = :red, markersize = 20)
  sm =
    surface!(ax, x, y, z; colormap = cmap, colorrange = (zmin, zmax), transparency = true)
  ...
  Label(fig[2, 1], text = plot_title)

  save("$FILES_DIR/$extension/$(generate_file_name("surface", extension))", fig)
  fig
end

# ╔═╡ b9de2c88-309c-4712-90ed-e13e65e2ec50
surface()

# ╔═╡ d77a878b-84e3-484c-a813-ee42d55ee062
md"""
_If Alice has $(@bind a Scrubbable(20)) apples, 
and she gives $(@bind b Scrubbable(3)) apples to Bob..._
"""

# ╔═╡ 5cfcb21f-9215-4afb-b097-c0c969a520ed
"El valor de a ahora es $a y el de b es $b"

# ╔═╡ ae481164-0631-4f4a-aedb-be7311fd4dd0
...
manifest_format = "2.0"
project_hash = "d870b44ba107d6ae8f4d88188d60e3fac8aa6eda"

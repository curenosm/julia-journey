...
# ╔═╡ 412fd3a5-30b8-477f-97e8-06ef83ca7f82
begin
	using ColorSchemes
    using DataFrames
    using FileIO
...
# ╔═╡ 26e6f59e-beee-4c45-bdce-6173350a7e3c
begin
...
# ╔═╡ 0ba4a366-404c-42b6-beca-bd6ed9958b1e
begin
	using AssociatedLegendrePolynomials
end

# ╔═╡ a33d5705-63fd-403f-8bf5-10b8b273ca4e
md"""
!!! success "Interactive tool to restart the notebook"
	With `Pluto` we can declare _HTML_ components to interact with the server and in that way refresh the changes.
"""

# ╔═╡ c65b542f-1d89-4a75-b60a-e9fc63b8a748
HTML("""
<!-- the wrapper span -->
<div>
...
# ╔═╡ 5980dc62-d3bb-4ed7-86be-6d59a58eeb70
begin
	# const REPO_DIR = "/Users/curenosm/Documents/github/julia-journey"
	const REPO_DIR = "D:/pc/github/journeys/julia-journey"
	const IMAGES_DIR = "$REPO_DIR/data_science/ch4_visualizing_data/makie/img"
end

# ╔═╡ 49a98ac8-14c7-4970-938e-25a4bf9cd8bf
TableOfContents(; title = "📚 Contents", 
				  indent = true,
				  depth = 4,
				  aside = true)

# ╔═╡ 2853b272-76ef-465b-82b1-04def9441ad2
md"""
# Quantum Numbers Visualization
"""

# ╔═╡ 991318d7-7e87-40bf-a16c-86e12fc5d0a2
md"""
Examples extracted from `Beautiful Makie`for learning purposes
"""

# ╔═╡ 7d6e2c1c-4f2e-47f2-8eae-4ddc84f46867
md"""
## Z-Number
"""

# ╔═╡ 696cec0f-708c-4e5c-9373-feffe43c10e5
function z_quantum_number()
	@info "Z Quantum Number"
	GLMakie.activate!()
	GLMakie.closeall()
...
# ╔═╡ 153d13f9-5e27-417f-9f60-3d50cecc0db9
z_quantum_number()

# ╔═╡ 9dd649df-2e7c-4c3a-9cb1-8a3ddcb02e3f
md"""
## Volume, Contour and Scatters
"""

# ╔═╡ 75f29d74-4770-4f1b-9b9a-2b0af4075413
function volume_contours_and_scatters()
	GLMakie.activate!()
	GLMakie.closeall()

	# Data
	x = y = z = -1:0.2:1
	vol₁ = [ix * iy * iz for ix in x, iy in y, iz in z]
	points_3d = [Point3f(ix, iy, iz) for ix in x, iy in y, iz in z]
	vol₂ = (vol₁ .+ 1) ./ 2 ## Scale everything to the interval 0,1
	cmap = :Hiroshige
	n = 101
	g(x) = x^2
	...
	# Configure
	volume!(ax₁, x, y, z, vol₂; colormap = cmap, transparency = true)
	contour!(ax₂, x, y, z, vol₁; 
		colormap = cmap, 
		alpha = 0.05,
		levels = [collect(-1:0.01:-0.3)..., collect(0.3:0.01:1)...]
	)
	meshscatter!(ax₃, vec(points_3d); color = vec(vol₁), colormap = cmap_alpha)
	meshscatter!(ax₄, vec(points_3d); 
		color = vec(vol₁), 
		colormap = cmap_alpha,
...
# ╔═╡ 73b68b4e-ccee-4cd7-ab26-16f19c1ff10a
volume_contours_and_scatters()

# ╔═╡ 7392114c-40fd-4e8a-a94a-678fe43ec171
md"""
## Tesseral Spherical
"""

# ╔═╡ d7353ec6-b23d-4486-9c86-e693113d20ed
function tesseral_spherical()
	GLMakie.activate!()
	GLMakie.closeall()
	...
	θ = LinRange(0, π, 200)
	φ = LinRange(0, 2π, 200)
	x = [sin(θ) * sin(φ) for θ in θ, φ in φ]
	y = [sin(θ) * cos(φ) for θ in θ, φ in φ]
	z = [cos(θ) for θ in θ, φ in φ]
	l = 10
	m = 1
	
	Ygrid = [Y(θ, φ, l, m) for θ in θ, φ in φ]
	Ylm = abs.(Ygrid)
	...
	# Configure
	fig[0, 1:2] = Label(fig, 
		"Tesseral Spherical Harmonics l = $l, m = $m",
		fontsize = 30,
		color = (:white, 0.85),
	)
...
# ╔═╡ c4d2192c-8938-48fb-b24d-51c04e6e163e
tesseral_spherical()

# ╔═╡ fe19297c-0d80-47de-a4ca-b8c4e67e5bda
md"""
## Contour 3D
"""

# ╔═╡ 841d5cc9-377a-4d28-b963-5e12c367bb7f
function contour_3d()
	GLMakie.activate!()
	GLMakie.closeall()
	...
	# Data
	x = y = z = 1..10 # Start, stop, interval
	xr = yr = zr = 1:10
	vol = [f(ix, iy, iz) for ix in xr, iy in yr, iz in zr]
...
# ╔═╡ b440b0c4-4cfc-4c5f-9c61-5823d0ea12f0
contour_3d()

# ╔═╡ b9d67c44-4176-4a95-afeb-0c8c6b219b0b
md"""
## Gauss 2D
"""

# ╔═╡ 6a00987b-f1f9-4a42-9e8d-d48deb32c6b4
function gauss_2d()
	# Setup
	GLMakie.activate!()
	GLMakie.closeall()
...
# ╔═╡ 9088e6e1-d389-4eb3-817b-181487b72ec4
gauss_2d()

# ╔═╡ 2d6f1b72-7d0b-476d-b381-09ce43abffce
md"""
## RRGraph 3D
"""

# ╔═╡ 35ed1810-9ad0-4441-8902-6e4e7d7cff60
begin
	
	"""
	Main Function to plot a beautiful graph.
	"""
	function rr_graph_3d()
		# Setup
		GLMakie.activate!()
		GLMakie.closeall()

		# Data
		adjacency_m_3d, x, y, z = RRGAdjacencyM3D()

		# Figure
		with_theme(theme_light()) do
			plot_graph_3d(adjacency_m_3d, x, y, z)
		end
	end
	...
	function RRGAdjacencyM3D(; radius = 0.17, nodes = 500, rseed = 123)
		...
		matrix_adj_diag = Diagonal(√2 * randn(nodes))
		matrix_adj = zeros(nodes, nodes)
		...
		for point in 1:nodes-1
			...
			if length(dindx) > 0
				...
				matrix_adj[point, dindx] = rnd
				matrix_adj[dindx, point] = rnd
			end
		end

		(matrix_adj .+ matrix_adj_diag, x, y, z)
	end
	
	"""
	"""
	function get_graph_edges_3d(adj_matrix_3d, x, y, z)
		xyzos = []
		weights = []
		for i in eachindex(x), j in i+1:length(x)
			if adj_matrix_3d[i, j] != 0.0
				push!(xyzos, [x[i], y[i], z[i]])
				push!(xyzos, [x[i], y[i], z[i]])
				push!(weights, adj_matrix_3d[i, j])
				push!(weights, adj_matrix_3d[i, j])
			end
		end
		(Point3f.(xyzos), Float32.(weights))
	end

	"""
	"""
	function plot_graph_3d(adjacency_m_3d, x, y, z)
		
		# Data
		cmap = (:Hiroshige, 0.75)
		adj_min = minimum(adjacency_m_3d)
		adj_max = maximum(adjacency_m_3d)
		diag_values = diag(adjacency_m_3d)
		segm, weights = get_graph_edges_3d(adjacency_m_3d, x, y, z)

		# Figure
		fig, ax, plt = linesegments(segm; 
			color = weights, 
			colormap = cmap,
			figure = (;
				size = (1200, 800),
		...
		meshscatter!(ax, x, y, z; 
			color = diag_values,
			markersize = abs.(diag_values) ./ 90,
			colorrange = (adj_min, adj_max),
			color_map = cmap,
		)
		Colorbar(fig[1, 2], plt, label = "weights", height = Relative(0.5))
...
# ╔═╡ 9533cc1c-49c1-4b2f-a13e-a55393c9355b
rr_graph_3d()
...
manifest_format = "2.0"
project_hash = "d58b98e3777f996018730b4921b488da8ac43995"
